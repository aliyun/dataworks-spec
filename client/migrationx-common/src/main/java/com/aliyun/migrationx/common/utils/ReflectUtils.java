...
public class ReflectUtils {
    ...
    private static boolean checkGetter(Class clz, Field field) {
        List<String> methods = getPossibleGetter(field);
        return methods.stream().anyMatch(m -> {
            try {
                Method method = ReflectionUtils.findMethod(clz, m);
                return method != null;
            } catch (Throwable e) {
                return false;
            }
    ...
    private static boolean checkSetter(Class clz, Field field) {
        List<String> methods = getPossibleSetter(field);
        return methods.stream().anyMatch(m -> {
            try {
                Method method = ReflectionUtils.findMethod(clz, m, field.getType());
                return method != null;
            } catch (Throwable e) {
                return false;
            }
